...
module internal {
    ...
    def check_envs [] {
        ...
        if ($invalid_env | is-not-empty) {
            fatal ec $"envs not found: ($invalid_env)"
        }
    }
...
export def ec [
    ...
    if ($target | is-empty) {
        fatal ec "target not set" "* Use `ec <target>` to edit targeted config\n* Use `ec --print-targets/-p to check all configs available"
    }

    let target_path = all_targets | where $it.target == $target | get path
    if ($target_path | is-empty) {
        fatal ec "invalid target" $"invalid target ($target)"
    }
